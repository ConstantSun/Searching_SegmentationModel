...
def visualize_to_tensorboard(test_loader, val_loader, train_loader_un_shuffle, writer, device,
                                        net, n_channels, n_classes, batch_size, epoch):
    for batch_idx, (batch_test, batch_val, batch_train) in enumerate(
            zip(test_loader, val_loader, train_loader_un_shuffle)):
        if batch_idx > 3:  # take 4 batches for visualizing.
            break

        # visualize the first batch in test, val, train set.
        for set_idx, batch in enumerate([batch_test, batch_val, batch_train]):
            imgs = batch['image']
            y_true = batch['mask']
